...
public class Itemmodelselector extends Button{
	...
	String modelTexture = "";
	Node modelNode;
	List<Geometry> modelList = new ArrayList<Geometry>();
	
	public Itemmodelselector(Object[] Arguments, Node ParentNode, float ScreenWidth, float ScreenHeight) 
	{
		...
		folderName = (String) Arguments[17];
		modelTexture = (String) Arguments[18];
		modelNode = new Node(buttonName + "Models");
	...
	{
		modelList = VoxelModels.generateModelsFromFolder(folderName, modelTexture);
	}
	...
	public void attachModels()
	{
		...
		Vector3f newSize = new Vector3f(sxx,syy,1);
		
		for(int i = 0; i < modelList.size(); i++)
		{
			float tfx = position.getX() + (sxx* (i/sy));
			float tfy = position.getY() + (-syy *(i%sy) + size.y - syy);
			
			modelList.get(i).setLocalTranslation(tfx, tfy, pz + 0.1f);
			
			Vector3f worldScale = modelList.get(i).getWorldScale();
			
			BoundingBox boundingBox = (BoundingBox) modelList.get(i).getModelBound();
			
			Vector3f bounding = new Vector3f(boundingBox.getXExtent(), boundingBox.getYExtent() , boundingBox.getZExtent());
			
			Vector3f modelNewSize = newSize.divide(worldScale.mult(bounding)).mult(0.25f);
			
			modelList.get(i).setLocalScale( modelNewSize);
			
			modelNode.attachChild(modelList.get(i));
		}
		
		buttonNode.attachChild(modelNode);
	}
